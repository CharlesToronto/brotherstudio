...
begin
  if to_regclass('public.dashboard_projects') is null then
    raise notice 'Skipping dashboard_projects payment status migration because dashboard_projects is missing.';
    return;
  end if;
  ...
  set payment_status = case
    ...
    when status = 'À facturer' then 'À facturer'
    when invoiced_amount > 0 and upcoming_amount <= 0 then 'Reçu'
    when upcoming_amount > 0 and invoiced_amount <= 0 then 'À facturer'
    when upcoming_amount > 0 and invoiced_amount > 0 then 'À facturer'
    else 'À facturer'
  end
